...
export function ServiceCard({ service, compact = false }: ServiceCardProps) {
  return (
    <article className={`group relative flex h-full flex-col overflow-hidden rounded-[8px] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:border-cyan/50 hover:shadow-xl ${compact ? "p-5" : "p-6"}`}>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className={`mt-3 font-semibold text-ink ${compact ? "text-lg leading-6" : "text-xl"}`}>{service.title}</h3>
        </div>
      ...
      <p className={`mt-4 text-sm text-slate-600 ${compact ? "leading-6" : "leading-7"}`}>{service.summary}</p>
      {!compact ? (
      ...
      <div className={`mt-auto flex flex-wrap gap-2 ${compact ? "pt-5" : "pt-6"}`}>
        <Link
          className={`inline-flex items-center gap-2 rounded-full bg-ink font-semibold text-white transition hover:bg-ocean ${compact ? "px-3 py-2 text-xs" : "px-4 py-2 text-sm"}`}
        ...
        <a
          className={`inline-flex items-center gap-2 rounded-full border border-slate-200 font-semibold text-ink transition hover:border-gold ${compact ? "px-3 py-2 text-xs" : "px-4 py-2 text-sm"}`}
