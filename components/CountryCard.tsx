...
export function CountryCard({ country }: CountryCardProps) {
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-[8px] border border-white/10 bg-white/10 p-6 text-white backdrop-blur transition hover:-translate-y-1 hover:border-cyan/60 hover:bg-white/20">
      <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-cyan via-gold to-white opacity-70" />
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan/90">
            {country.region}
          </p>
      ...
      <div className="mt-auto flex flex-wrap gap-3 pt-6">
        <Link
          className="inline-flex items-center gap-2 rounded-full bg-cyan px-4 py-2 text-sm font-semibold text-midnight transition hover:bg-white"
        ...
        <a
          className="inline-flex items-center gap-2 rounded-full border border-white/20 px-4 py-2 text-sm font-semibold text-white transition hover:border-cyan"
